...
// 获取案例列表
export const getCases = async (params: CaseFilterParams = {}): Promise<CaseListResponse> => {
  try {
    ...
    if (params.categoryId) {
      cases = cases.filter((caseItem: Case) => caseItem.categoryId === params.categoryId);
    }
    if (params.serviceId) {
      cases = cases.filter((caseItem: Case) => caseItem.serviceId === params.serviceId);
    }
    ...
    if (params.status) {
      cases = cases.filter((caseItem: Case) => caseItem.status === params.status);
    }
    if (params.featured !== undefined) {
      cases = cases.filter((caseItem: Case) => caseItem.featured === params.featured);
    }
    if (params.area) {
      if (params.area.min !== undefined) {
        cases = cases.filter((caseItem: Case) => (caseItem.area || 0) >= params.area!.min!);
      }
      if (params.area.max !== undefined) {
        cases = cases.filter((caseItem: Case) => (caseItem.area || 0) <= params.area!.max!);
      }
    }
    if (params.style) {
